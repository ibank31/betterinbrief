...
const TwoTracks: React.FC<DeviceProps> = ({surface, seed, data}) => {
  ...
  return <div style={{position: "absolute", right: 34, top: 495 + Math.round(rand(seed, "top") * 45), width: 520, height: 470, color: ink, opacity: 0.94, transform: `translateX(${(1 - p) * 85}px)`}}>
    ...
    <div style={{position: "absolute", left: 240, top: 180, width: heroLen, height: 8, background: colors.orange, transform: `rotate(${downAngle}deg)`, transformOrigin: "left"}} />
    {Array.from({length: cardCount}).map((_, index) => <Card key={index} x={50 + index * (36 + Math.round(rand(seed, "cx" + index) * 16)) + drift * 16} y={75 + index * (46 + Math.round(rand(seed, "cy" + index) * 16))} rotate={-16 + rand(seed, "cr" + index) * 9} opacity={0.35 + index * 0.18} />)}
    {[0, 1, 2].map((index) => <div key={index} style={{position: "absolute", left: 282 + Math.round(rand(seed, "dx" + index) * 130), top: 202 + index * (64 + Math.round(rand(seed, "dy" + index) * 24)), width: 30 + Math.round(rand(seed, "ds" + index) * 16), height: 30 + Math.round(rand(seed, "ds" + index) * 16), borderRadius: "50%", background: index === 2 ? colors.orange : ink, transform: `scale(${0.75 + p * 0.25})`}} />)}
  </div>;
};
...
const EvidenceScan: React.FC<DeviceProps> = ({surface, seed}) => {
  ...
  return <div style={{position: "absolute", right: 44, bottom: 285 + Math.round(rand(seed, "bottom") * 40), width: 430, height: 530, color: ink, opacity: 0.9}}>
    <div style={{position: "absolute", inset: 0, border: `3px solid ${ink}`, background: soft, transform: `rotate(${tilt}deg)`}} />
    {Array.from({length: lineCount}).map((_, index) => <div key={index} style={{position: "absolute", left: 55, top: 80 + index * Math.round(300 / lineCount), width: index === hot ? 260 : 190 + Math.round(rand(seed, "lw" + index) * 125), height: 10, background: index === hot ? colors.orange : ink, opacity: index === hot ? 0.86 : 0.5}} />)}
    ...
    <div style={{position: "absolute", right: stampRight, top: 40, width: ring, height: ring, border: `6px solid ${colors.orange}`, borderRadius: "50%"}} />
    <div style={{position: "absolute", right: stampRight - 23, top: 135, width: 62, height: 7, background: colors.orange, transform: "rotate(45deg)"}} />
  </div>;
};
...
const DecisionGraph: React.FC<DeviceProps> = ({surface, seed, data}) => {
  ...
  return <div style={{position: "absolute", right: 22, top: 640 + Math.round(rand(seed, "top") * 60), width: 530, height: 520, color: ink, opacity: 0.78}}>
    ...
    {nodes.map((node, index) => <div key={index} style={{position: "absolute", left: node.x, top: node.y, width: node.s, height: node.s, borderRadius: "50%", border: `5px solid ${index >= count - 2 ? colors.orange : ink}`, background: index === count - 1 ? colors.orange16 : "transparent", transform: `scale(${0.6 + pop * 0.4})`, transformOrigin: "center"}} />)}
  </div>;
};
...
const TaskSystem: React.FC<DeviceProps> = ({surface, seed, data}) => {
  ...
  return <div style={{position: "absolute", right: 22, top: 535, width: 360, height: 720, color: ink, opacity: 0.84}}>
    {Array.from({length: count}).map((_, index) => {
      ...
      return <div key={index} style={{position: "absolute", right: index % 2 ? 38 : 0, top: index * step, width: 195 + Math.round(rand(seed, "w" + index) * 45), height: Math.min(92, step - 22), border: `3px solid ${accent ? colors.orange : ink}`, transform: `translateX(${(1 - p) * (index % 2 ? 58 : -58)}px)`, background: index === 0 && accent ? colors.orange16 : "transparent"}} />;
    })}
    <div style={{position: "absolute", left: 120 + Math.round(rand(seed, "spine") * 30), top: 78, height: Math.max(200, step * (count - 1)), width: 5, background: ink, opacity: 0.5}} />
  </div>;
};
...
const PrioritySignal: React.FC<DeviceProps> = ({surface, seed, data}) => {
  ...
  if (values.length > 0) {
    ...
    return <div style={{position: "absolute", right: 46, bottom: 250, width: 400, height: 500, color: ink, opacity: 0.86}}>
      <div style={{position: "absolute", left: 34, right: 12, bottom: 30, height: 4, background: ink, opacity: 0.55}} />
      {values.map((value, index) => <div key={index} style={{position: "absolute", left: 45 + index * gap, bottom: 34, width: barWidth, height: Math.max(24, (value / scale) * 370) * rise, background: index === hero ? colors.white : ink, border: index === hero ? `4px solid ${colors.black}` : "none"}} />)}
    </div>;
  }
  // Tanpa data nyata: metafora sinyal non-kuantitatif — tanpa batang/sumbu.
  return <div style={{position: "absolute", right: 46, bottom: 250, width: 400, height: 500, color: ink, opacity: 0.86}}>
    <div style={{position: "absolute", left: 30 + Math.round(rand(seed, "bx") * 40), top: 60, width: 300, height: 5, background: colors.black, transform: `rotate(-${25 + rand(seed, "ba") * 16}deg)`, transformOrigin: "left", opacity: rise}} />
    {[0, 1, 2].map((index) => <div key={index} style={{position: "absolute", left: 60 + Math.round(rand(seed, "mx" + index) * 250), top: 150 + index * 105 + Math.round(rand(seed, "my" + index) * 45), width: 26 + Math.round(rand(seed, "ms" + index) * 20), height: 26 + Math.round(rand(seed, "ms" + index) * 20), background: index === 2 ? colors.white : "transparent", border: `4px solid ${index === 2 ? colors.black : ink}`, transform: `rotate(45deg) scale(${0.6 + rise * 0.4})`}} />)}
  </div>;
};
