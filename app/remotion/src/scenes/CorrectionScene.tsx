...
export const CorrectionScene: React.FC<CorrectionSceneProps> = ({eyebrow, misconception, correction, symbol = "≠", subtitle, world, surface}) => {
  ...
  // v1.3d: koreksi mendarat dengan fisika spring; simbol dan garis aksen
  // berdenyut bergantian setelah entrance agar scene tidak membeku.
  const answerSpring = spring({frame: frame - 16, fps, config: motion.spring.editorial});
  const beatSym = microBeat(frame, {start: 58, period: 76});
  const beatBar = microBeat(frame, {start: 58, period: 76, phase: 38});
  ...
  return <EditorialFrame background={bg} color={ink} world={world} surface={s}>
    ...
    <div style={{position: "absolute", left: safeZones.left, top: safeZones.top, width: 880}}>
      ...
      <div style={{position: "absolute", right: 18, top: 190, fontSize: 190, lineHeight: 1, fontWeight: typography.weight.black, color: accent, transform: `rotate(${(1-cut)*12 - beatSym * 4}deg) scale(${.8+.2*cut + beatSym * 0.05})`, opacity: cut}}>{symbol}</div>
      <div style={{marginTop: 128, marginLeft: 96, maxWidth: 760, fontSize: 96, lineHeight: typography.lineHeight.hero, letterSpacing: typography.letterSpacing.tight, fontWeight: typography.weight.black, opacity: answer, transform: `translateY(${(1-answerSpring)*50}px)`}}>{correction}</div>
    </div>
    <div style={{position: "absolute", left: 0, top: 1040, height: 18, width: `${cut*790 + beatBar * 36}px`, background: accent}}/>
